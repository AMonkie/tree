...
TP3.Physics = {
    ...
    // --- 6. Mise à jour de la position de p1 sans changer la taille ---
    const length = node.p1.clone().sub(node.p0).length();
    node.p1.copy(node.p0).add(newDirection.multiplyScalar(length));
